...
async function main() {
  ...
  // Query the task details for Task ID 1 (or any other task ID)
  const taskId = 1;  // Change to the task ID you want to confirm
  const [description, status] = await contract.getTaskStatus(taskId);

  const statusString = (status === 0) ? "Not Started" : (status === 1) ? "In Progress" : "Completed";
  console.log(`Task ID ${taskId} - Description: "${description}", Status: ${statusString}`);
}
